...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const AVATAR_BASE = "https://api.dicebear.com/7.x/bottts/svg?seed=";
    ...
    function avatarUrl(seed) {
        return `${AVATAR_BASE}${encodeURIComponent(seed || "Player")}`;
    }
    ...
    function renderHeaderAndForm(user) {
        ...
        if (settingsAvatar) {
            settingsAvatar.src = avatarUrl(user?.avatarSeed || user?.username);
        }
    ...
    function setupEventListeners() {
        ...
        if (darkThemeToggle) {
            darkThemeToggle.addEventListener("change", () => {
                ...
                // Apply immediately — no API round-trip needed for visual feedback
                if (window.CodingArenaTheme) {
                    window.CodingArenaTheme.applyTheme(newTheme);
                }
                // Also cache locally so other pages pick it up right away
                try {
                    const stored = JSON.parse(localStorage.getItem("ca_preferences") || "{}");
                    stored.theme = newTheme;
                    localStorage.setItem("ca_preferences", JSON.stringify(stored));
                } catch (e) { /* ignore */ }
