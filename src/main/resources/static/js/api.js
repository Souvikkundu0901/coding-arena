...
async function updatePreferences(preferences) {
  if (MOCK_MODE) {
    ...
    try {
      const updated = await apiRequest("/users/me/preferences", "PATCH", preferences);
      try {
        const stored = JSON.parse(localStorage.getItem("ca_preferences") || "{}");
        localStorage.setItem("ca_preferences", JSON.stringify({ ...stored, ...updated }));
...
window.CodessAPI = window.CodingArenaAPI;